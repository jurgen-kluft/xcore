...
template<class T>
xstring				xstring_const_base<T>::mid(s32 inPosition, s32 inNum) const
{
	xstring outMid;
	const s32 l = getLength();
	XBOUNDS(inPosition, 0, l);												// Must start to grab within string
...
template<class T> 
xstring				xstring_const_base<T>::substr(s32 inPosition, s32 inNum) const
{
...
template <class T>
void			xstring_mutable_base<T>::erase (s32 inStart, s32 inLength)
{
	if (inLength == -1)
		inLength = __const_base::getLength()- inStart;
